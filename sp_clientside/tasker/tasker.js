...
AFC.Util =
	...
	(function (win, undefined) {
		...
		return {
			loadScriptNJQ: loadJQ.bind(null, options.jqUrl),
			loadScript: loadJQ,
			init: init,
...
(function () {
	AFC.TaskerDetail = (function (win, undefined) {
		function setInit() {
			$("#onetidinfoblockV").closest("td.ms-toolbar").hide();
			$("h3:contains('OCR')").closest("tr").hide();
			$("h3:contains('Closed')").closest("tr").hide();
		}

		function init() {
			var util = AFC.Util;
			util.loadScriptNJQ(AFC.TaskerDetail.setInit);
		}
	...
	_spBodyOnLoadFunctionNames.push("AFC.TaskerDetail.init");
})();
